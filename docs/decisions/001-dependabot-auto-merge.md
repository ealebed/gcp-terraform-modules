...
This monorepo’s CI ([`.github/workflows/ci-test-and-prepare-release.yaml`](../../.github/workflows/ci-test-and-prepare-release.yaml)) requires **exactly one** of `major` / `minor` / `patch` / `no-release`. Dependabot PRs must keep `no-release` and must not also receive another release-type label.
...
- Dependabot must keep the `no-release` label and must not also have `major` / `minor` / `patch`.
